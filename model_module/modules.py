...
def ResInput(inputs, filters, kernel_size, scope, reuse=None):
    ...
    '''
    with tf.variable_scope(scope, reuse=reuse):
        outputs = tf.layers.conv2d(inputs, filters, kernel_size, strides=(1, 1), padding="SAME", name="conv_input",
                                   reuse=reuse)
        return outputs
...
def ResOutput(inputs, filters, kernel_size, scope, reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        outputs = tf.layers.conv2d(inputs, filters, kernel_size, strides=(1, 1), padding="SAME", name="conv_out",
                                   reuse=reuse)
        return outputs


def Fusion(closeness_output, period_output, trend_output, outside_condition, scope, shape):
    '''
    ...
    with tf.variable_scope(scope):
        ...
        Wc = tf.get_variable("closeness_matrix", dtype=tf.float32, shape=shape,
                             initializer=tf.contrib.layers.xavier_initializer(), trainable=True)
        Wp = tf.get_variable("period_matrix", dtype=tf.float32, shape=shape,
                             initializer=tf.contrib.layers.xavier_initializer(), trainable=True)
        Wt = tf.get_variable("trend_matrix", dtype=tf.float32, shape=shape,
                             initializer=tf.contrib.layers.xavier_initializer(), trainable=True)

        output = tf.reshape(closeness_output,
                            [closeness_output.shape[0] * closeness_output.shape[1], closeness_output.shape[2]])
        output = tf.matmul(output, Wc)
        closeness_output = tf.reshape(output,
                                      [closeness_output.shape[0], closeness_output.shape[1], closeness_output.shape[2]])
        ...
        outputs = tf.add(outputs,outside_condition)
